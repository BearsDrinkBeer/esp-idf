...
static atomic_bool spi_periph_claimed[SOC_SPI_PERIPH_NUM] = {
...
void spicommon_cs_initialize(spi_host_device_t host, int cs_io_num, int cs_num, int force_gpio_matrix, bool cs_active_high)
{
    ...
    else
    {
        ...
        if (cs_num == 0)
            esp_rom_gpio_connect_in_signal(cs_io_num, spi_periph_signal[host].spics_in, cs_active_high);
...
{
#define CHECK_IOMUX_PIN(HOST, PIN_NAME)                                           \
    if (GPIO.func_in_sel_cfg[spi_periph_signal[(HOST)].PIN_NAME##_in].sig_in_sel) \
    return false
